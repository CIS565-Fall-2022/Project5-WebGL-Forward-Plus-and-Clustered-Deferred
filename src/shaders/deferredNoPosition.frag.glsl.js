...
export default function(params) {
  ...
  uniform mat4 u_viewInv; // for depth
  uniform float u_fov;
  ...
  void main() {
    ...
    vec4 colorAndDepth = texture2D(u_gbuffers[0], v_uv);
    vec3 albedo = colorAndDepth.rgb;
    ...
    // add camera.left and camera.up
    vec3 position = position_of_center + normalize(u_viewInv[0].xyz) * ndc.x * half_width
      + normalize(u_viewInv[1].xyz) * ndc.y * half_height;
    ...
    // gl_FragColor = vec4(position_of_center, 1.0) / 10.0;
  }
  `;
}