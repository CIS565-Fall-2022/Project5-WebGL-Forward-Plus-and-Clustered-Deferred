...
import toTextureVert from '../shaders/deferred_O2_ToTexture.vert.glsl';
import toTextureFrag from '../shaders/deferred_O2_ToTexture.frag.glsl';
import QuadVertSource from '../shaders/quad_O2.vert.glsl';
import fsSource from '../shaders/deferred_O2_GGX.frag.glsl.js';
import TextureBuffer from './textureBuffer';
import BaseRenderer, { MAX_LIGHTS_PER_CLUSTER } from './base';

export const NUM_GBUFFERS = 2;

// Packs g-buffer components into vec4s (no seperate buffer for each property)
// Also recreates normal from 2 values in vector (saves a gbuffer texture)


export default class ClusteredDeferredRenderer_O2_GGX extends BaseRenderer {
  constructor(xSlices, ySlices, zSlices, roughness, metallic) {
    ...
    this._progShade = loadShaderProgram(QuadVertSource, fsSource({
      ...
      uniforms: ['u_gbuffers[0]', 'u_gbuffers[1]', 'u_lightbuffer', 'u_clusterbuffer', 'u_canvasresolution', 'u_clusterslices', 'u_viewMatrix', 'u_viewProjectionInv', 'u_camPos', 
      'u_matProperties'],
      attribs: ['a_uv'],
    });
    ...
    this._viewProjectionInv = mat4.create();
  ...
  render(camera, scene) {
    ...
    mat4.invert(this._viewProjectionInv, this._viewProjectionMatrix);
    ...
    // Upload the camera matrix inverse
    gl.uniformMatrix4fv(this._progShade.u_viewProjectionInv, false, this._viewProjectionInv);
    gl.uniform3fv(this._progShade.u_camPos, camera.position);
    gl.uniform4fv(this._progShade.u_matProperties, vec4.fromValues(this._roughness, this._metallic, 0, 0));
